...
exports.extractRenameSuggestions = extractRenameSuggestions;
exports.stripRenameBlock = stripRenameBlock;
exports.streamChatWithAI = streamChatWithAI;
exports.testTextAI = testTextAI;
...
const SUB_CAT_NAME = Object.fromEntries(models_1.SUB_CATEGORIES.map((c) => [c.id, c.name]));
const CYCLE_NAME = {
    weekly: '周',
    monthly: '月',
    quarterly: '季',
...
function formatItemMeta(item) {
    ...
    if (item.expiry)
        parts.push(`保质至 ${item.expiry}`);
    if (item.openedAt)
        parts.push(`已开封 ${item.openedAt}`);
    if (item.warrantyMonths)
        parts.push(`保 ${item.warrantyMonths} 月起 ${item.purchasedAt || '?'}`);
    if (item.minStock != null)
        parts.push(`min=${item.minStock}`);
    if (item.season)
        parts.push(`季 ${item.season}`);
    return parts.length ? `（${parts.join('，')}）` : '';
}
function formatItemList(items, limit = 14) {
    if (!items.length)
        return '（暂无物品）';
    const head = items
        .slice(0, limit)
    ...
    const tailTags = new Map();
    for (const it of items.slice(limit)) {
        for (const tag of it.tags || [])
            tailTags.set(tag, (tailTags.get(tag) || 0) + 1);
    }
    const tagSummary = Array.from(tailTags.entries())
        ...
        .map(([t, n]) => `${t}×${n}`)
        .join('、');
    return `${head}… 等另 ${items.length - limit} 件${tagSummary ? `（标签：${tagSummary}）` : ''}`;
}
function eventLine(ev) {
    const due = ev.daysLeft < 0
        ? `已逾期 ${-ev.daysLeft} 天`
        : ev.daysLeft === 0
            ? '今天'
            : `${ev.daysLeft} 天后`;
    return `- [${ev.level}|${models_1.REMINDER_KIND_LABEL[ev.kind] || ev.kind}] ${ev.title} · ${ev.subtitle} · ${due}`;
}
function buildChatSystemPrompt(rooms, cabinets, items, subscriptions = [], ctx = {}, itemLists = []) {
    const today = ctx.today || new Date().toISOString().slice(0, 10);
    // Group items by cabinetId (every item, no filters).
    const itemsByCabinet = new Map();
    for (const it of items) {
        const key = it.cabinetId || `__no_cabinet_${it.roomId}`;
        if (!itemsByCabinet.has(key))
            itemsByCabinet.set(key, []);
        itemsByCabinet.get(key).push(it);
    }
    const placed = items.filter((i) => i.status === 'placed');
    const pending = items.filter((i) => i.status === 'pending');
    const totalCabinets = cabinets.filter((c) => !(0, cabinet_1.isLooseCabinet)(c)).length;
    // ===== Per-room layout (includes both normal and loose cabinets) =====
    const roomBlocks = rooms.map((room) => {
        const normalCabs = cabinets.filter((c) => c.roomId === room.id && (!c.type || c.type === 'normal'));
        const looseCab = cabinets.find((c) => c.roomId === room.id && c.type === 'loose');
        const lines = [];
        ...
        return `### ${room.name}（${normalCabs.length} 个储物单元${ctx.currentRoomId === room.id ? '，← 当前正在浏览' : ''}）\n${lines.join('\n')}`;
    });
    // 全屋自由区
    ...
    if (subscriptions.length) {
        ...
        const monthly = active.reduce((sum, s) => sum + (0, subscription_1.subscriptionMonthlyCost)(s), 0);
        const formatSubLine = (s) => {
            ...
            return `- [subId=${s.id}] ${s.name}（${cat}）¥${s.amount.toFixed(2)}/${cycle} · ${due}${extras.length ? ` · ${extras.join(' · ')}` : ''}`;
        };
        const subLines = active
            .slice()
            .sort((a, b) => new Date(a.nextDueAt || '2999-01-01').getTime() -
            new Date(b.nextDueAt || '2999-01-01').getTime())
            .slice(0, 30)
            .map(formatSubLine);
        const pausedLines = paused.slice(0, 12).map(formatSubLine);
        subBlock = `\n【订阅（生效 ${active.length} 条，月均 ¥${monthly.toFixed(0)}${paused.length ? `；另暂停 ${paused.length} 条` : ''}）】\n${subLines.join('\n') || '（无生效订阅）'}${pausedLines.length ? `\n暂停/取消：\n${pausedLines.join('\n')}` : ''}`;
    }
    // ===== Reminder events =====
    const itemEvents = (0, reminder_1.computeItemEvents)(items);
    const subEvents = (0, reminder_1.computeSubscriptionEvents)(subscriptions);
    const allEvents = [...itemEvents, ...subEvents].sort((a, b) => a.daysLeft - b.daysLeft);
    const importantEvents = allEvents.filter((e) => e.level === 'critical' || e.level === 'warn');
    let eventsBlock = '';
    ...
    if (itemLists.length) {
        const lines = itemLists.slice(0, 30).map((l) => {
            ...
            const sample = validIds
                ...
                .filter(Boolean)
                .join('、');
            return `- [listId=${l.id}] ${l.emoji || '🧳'} ${l.name}（${validIds.length} 件${sample ? `：${sample}${validIds.length > 4 ? '…' : ''}` : ''}）`;
        });
        listsBlock = `\n【物品清单（共 ${itemLists.length} 个，手动维护）】\n${lines.join('\n')}`;
    }
    // ===== Current location =====
    const locationParts = [`今天是 ${today}`];
    if (ctx.currentPath)
        locationParts.push(`用户当前在页面 ${ctx.currentPath}`);
    if (ctx.currentRoomId && ctx.currentRoomId !== models_1.GLOBAL_ROOM_ID) {
        const r = rooms.find((rm) => rm.id === ctx.currentRoomId);
        if (r)
            locationParts.push(`正在浏览房间「${r.name}」`);
    }
...
function extractRenameSuggestions(text) {
...
function stripRenameBlock(text) {
...
async function streamChatWithAI(storage, messages, onDelta, abortSignal) {
    let lastError;
    try {
        return await streamOpenAiCompat('deepseek', DEEPSEEK_API, undefined, (await (0, indexeddb_1.getConfig)(storage, 'deepseekModel', '')) || undefined, messages, onDelta, abortSignal);
    }
    catch (err) {
    ...
    try {
        return await streamOpenAiCompat('minimax', MINIMAX_API, undefined, (await (0, indexeddb_1.getConfig)(storage, 'minimaxModel', '')) || DEFAULT_MINIMAX_MODEL, messages, onDelta, abortSignal);
    }
    catch (err) {
        if (abortSignal?.aborted)
            throw err;
    ...
    try {
        return await streamOpenAiCompat('openrouter', OPENROUTER_API, undefined, (await (0, indexeddb_1.getConfig)(storage, 'openrouterModel', '')) || undefined, messages, onDelta, abortSignal);
    }
    catch (err) {
        if (abortSignal?.aborted)
            throw err;
...
async function testTextAI(storage, provider) {
    const modelName = provider === 'deepseek'
        ? 'deepseekModel'
        : provider === 'minimax'
            ? 'minimaxModel'
            : 'openrouterModel';
    const model = (await (0, indexeddb_1.getConfig)(storage, modelName, '')) ||
    ...
    const res = await fetch(userKey ? url : (0, apiBase_1.apiUrl)(`/api/ai/${provider}`), {
        ...
        headers: {
            'Content-Type': 'application/json',
            ...(userKey ? { Authorization: `Bearer ${userKey}` } : (0, apiBase_1.apiAuthHeaders)()),
        },
        body: JSON.stringify({
            model,
    ...
    if (!res.ok)
        throw new Error(`连接失败 (${res.status})：${(await res.text()).slice(0, 120)}`);
    return '连接正常';
}
