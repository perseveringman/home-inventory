...
exports.suggestItemDraft = suggestItemDraft;
...
const TAG_NAMES = models_1.PRESET_TAGS.map((tag) => tag.name);
const KEYWORD_TAGS = [
...
function buildPlacementOptions(rooms, cabinets, items) {
    const itemsByCabinet = new Map();
    for (const item of items) {
        if (!itemsByCabinet.has(item.cabinetId))
            itemsByCabinet.set(item.cabinetId, []);
        itemsByCabinet.get(item.cabinetId).push(item);
    }
    const lines = [
        `- roomId=${models_1.GLOBAL_ROOM_ID} cabinetId=__global_loose__ label=全屋自由区`,
    ];
...
7. cabinetId 为普通柜子 id 时，roomId 必须是该柜子的 roomId；选择房间自由区时 cabinetId 用 "__room_loose__"；选择全屋自由区时 roomId 用 "${models_1.GLOBAL_ROOM_ID}" 且 cabinetId 用 "__global_loose__"。`;
}
...
function sanitizeNullablePositiveNumber(value, max) {
    if (value === null)
        return null;
...
function sanitizeSuggestion(raw, rooms, cabinets, mode) {
    const obj = (raw && typeof raw === 'object' ? raw : {});
    const roomIds = new Set([models_1.GLOBAL_ROOM_ID, ...rooms.map((room) => room.id)]);
    ...
    else if (cabinetId === '__global_loose__' || roomId === models_1.GLOBAL_ROOM_ID) {
        roomId = models_1.GLOBAL_ROOM_ID;
        cabinetId = '__global_loose__';
    }
    else if (cabinetId === '__room_loose__') {
        if (!roomId || roomId === models_1.GLOBAL_ROOM_ID || !roomIds.has(roomId)) {
            roomId = undefined;
            cabinetId = undefined;
        }
    }
...
function scoreCabinet(cabinet, tags, item, allItems) {
    ...
    for (const hint of ROOM_TAG_HINTS) {
        if (!hint.tags.some((tag) => tags.includes(tag)))
            continue;
        for (const word of hint.words) {
            if (cabinetText.includes(word.toLowerCase()))
                score += 2;
        }
    }
...
function heuristicSuggestion(input) {
    ...
    else if (input.item.roomId && input.item.roomId !== models_1.GLOBAL_ROOM_ID) {
        suggestion.roomId = input.item.roomId;
        suggestion.cabinetId = '__room_loose__';
    }
    else {
        suggestion.roomId = models_1.GLOBAL_ROOM_ID;
...
function inferSeason(name) {
    if (/羽绒|毛衣|围巾|冬/.test(name))
        return 'winter';
    if (/短袖|凉鞋|泳|夏/.test(name))
        return 'summer';
    if (/春/.test(name))
        return 'spring';
    if (/秋/.test(name))
...
async function buildVisionMessages(prompt, item) {
    if (!item.image) {
        return [{ role: 'user', content: prompt }];
    }
    const base64 = await (0, image_1.fileToBase64)(item.image);
...
async function suggestItemDraft(storage, input) {
    const prompt = buildSuggestionPrompt(input);
    if (input.item.image) {
        try {
            const text = await callOpenAiCompat('minimax', MINIMAX_API, undefined, (await (0, indexeddb_1.getConfig)(storage, 'minimaxModel', '')) || DEFAULT_MINIMAX_MODEL, await buildVisionMessages(prompt, input.item));
        ...
        try {
            const text = await callOpenAiCompat('openrouter', OPENROUTER_API, undefined, (await (0, indexeddb_1.getConfig)(storage, 'openrouterModel', '')) || undefined, await buildVisionMessages(prompt, input.item));
            return sanitizeSuggestion(parseJsonObject(text), input.rooms, input.cabinets, 'ai');
        }
        catch (err) {
    ...
    try {
        const text = await callOpenAiCompat('deepseek', DEEPSEEK_API, undefined, (await (0, indexeddb_1.getConfig)(storage, 'deepseekModel', '')) || undefined, [{ role: 'user', content: prompt }]);
        return sanitizeSuggestion(parseJsonObject(text), input.rooms, input.cabinets, 'ai');
    }
    catch (err) {
    ...
    try {
        const text = await callOpenAiCompat('minimax', MINIMAX_API, undefined, (await (0, indexeddb_1.getConfig)(storage, 'minimaxModel', '')) || DEFAULT_MINIMAX_MODEL, [{ role: 'user', content: prompt }]);
        return sanitizeSuggestion(parseJsonObject(text), input.rooms, input.cabinets, 'ai');
    }
    catch (err) {
    ...
    try {
        const text = await callOpenAiCompat('openrouter', OPENROUTER_API, undefined, (await (0, indexeddb_1.getConfig)(storage, 'openrouterModel', '')) || undefined, [{ role: 'user', content: prompt }]);
        return sanitizeSuggestion(parseJsonObject(text), input.rooms, input.cabinets, 'ai');
    }
    catch (err) {
