...
exports.parseDetection = parseDetection;
exports.detectCabinetsAndItems = detectCabinetsAndItems;
/**
...
function ensureUsefulDetection(result) {
    if (result.cabinets.length === 0 && result.items.length === 0) {
        throw new Error('AI 返回了空识别结果：cabinets 和 items 都为空');
    }
    return result;
}
function buildDetectionRepairPrompt(err, rawText, round, maxRounds) {
...
async function parseDetectionWithRepair({ provider, initialMessages, callModel, maxRepairRounds, }) {
    ...
    throw new Error(`${provider} 识别结果修复失败（已尝试 ${maxRepairRounds} 轮）：${errorMessage(lastError)}`);
}
function imageDataUrl(blob, base64) {
    return `data:${blob.type || 'image/jpeg'};base64,${base64}`;
}
/* ---------- MiniMax M3 Vision ---------- */
async function callMiniMax(apiKey, model, messages) {
    const effectiveKey = apiKey || (0, apiBase_1.getUserApiKey)('minimax') || undefined;
    const res = await fetch(effectiveKey ? MINIMAX_API : (0, apiBase_1.apiUrl)('/api/ai/minimax'), {
        method: 'POST',
        headers: {
    ...
    if (data.error)
        throw new Error(data.error.message || 'MiniMax 返回错误');
    return data.choices?.[0]?.message?.content || '';
}
async function detectWithMiniMax(apiKey, model, blob, maxRepairRounds) {
...
async function callOpenRouter(apiKey, model, messages) {
    const effectiveKey = apiKey || (0, apiBase_1.getUserApiKey)('openrouter') || undefined;
    const res = await fetch(effectiveKey ? OPENROUTER_API : (0, apiBase_1.apiUrl)('/api/ai/openrouter'), {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            ...(effectiveKey ? { Authorization: `Bearer ${effectiveKey}` } : (0, apiBase_1.apiAuthHeaders)()),
    ...
    const data = await res.json();
    if (data.error)
        throw new Error(data.error.message || 'OpenRouter 返回错误');
    return data.choices?.[0]?.message?.content || '';
}
async function detectWithGemini(apiKey, model, blob, maxRepairRounds) {
    const base64 = await (0, image_1.fileToBase64)(blob);
    const initialMessages = [
        {
            role: 'user',
...
/* ---------- Claude Vision ---------- */
async function detectWithClaude(apiKey, blob, size, maxRepairRounds) {
    const maxSide = 1568;
    let imgBlob = blob;
    if (blob.size > 100 && Math.max(size.width, size.height) > maxSide) {
        ...
        if (bmp) {
            ...
            imgBlob = await new Promise((r) => c.toBlob((b) => r(b), 'image/jpeg', 0.85));
            bmp.close();
        }
    }
    const base64 = await (0, image_1.fileToBase64)(imgBlob);
...
async function callClaude(apiKey, messages) {
    const effectiveKey = apiKey || (0, apiBase_1.getUserApiKey)('claude') || undefined;
    const res = await fetch(effectiveKey ? ANTHROPIC_API : (0, apiBase_1.apiUrl)('/api/ai/claude'), {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            ...(effectiveKey
...
/* ---------- 启发式占位 ---------- */
function heuristicDetection(size) {
...
async function detectCabinetsAndItems(blob, size, cfg) {
    const maxRepairRounds = normalizeRepairRounds(cfg.maxRepairRounds);
    try {
        return await detectWithMiniMax(cfg.minimaxKey, cfg.minimaxModel || DEFAULT_MINIMAX_MODEL, blob, maxRepairRounds);
    }
    catch (e) {
        console.warn('MiniMax detect failed after repair, try next provider/fallback:', e);
    }
    try {
        return await detectWithGemini(cfg.openrouterKey, cfg.openrouterModel || 'google/gemini-2.5-flash', blob, maxRepairRounds);
    }
    catch (e) {
