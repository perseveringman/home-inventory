...
exports.subscriptionCycleDays = subscriptionCycleDays;
exports.subscriptionMonthlyCost = subscriptionMonthlyCost;
exports.advanceSubDue = advanceSubDue;
exports.renewSubDue = renewSubDue;
exports.findDuplicateSubscriptions = findDuplicateSubscriptions;
exports.latestPriceChange = latestPriceChange;
exports.buildSubscriptionInsights = buildSubscriptionInsights;
const models_1 = require("../models");
function subscriptionCycleDays(sub) {
    const preset = models_1.SUB_CYCLES.find((c) => c.id === sub.cycle);
    if (preset && sub.cycle !== 'custom')
        return preset.days;
    if (sub.cycle === 'custom' && sub.cycleDays && +sub.cycleDays > 0)
        return +sub.cycleDays;
    return 30;
}
/** 月均 = 金额 * 30 / 周期天数 */
...
const CAT_NAME = Object.fromEntries(models_1.SUB_CATEGORIES.map((c) => [c.id, c.name]));
...
function latestPriceChange(sub) {
    ...
    if (!prev.amount || cur.amount <= prev.amount)
        return null;
    return {
        from: prev.amount,
        to: cur.amount,
...
function buildSubscriptionInsights(subscriptions, now = Date.now()) {
    ...
    const hikes = active
        ...
        .filter((item) => !!item.change && item.change.pct >= 10)
