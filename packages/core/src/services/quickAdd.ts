import type { Cabinet, Item, ItemStatus, Photo, Room } from '../models';
...
function tidyItemName(name: string): string {
  return name
...
function parseQuantityText(text: string): number | null {
...
function parseNaturalItemPhrase(phrase: string): ParsedQuickAddLine | null {
  let value = phrase
    .trim()
    .replace(/^[,，、;；\s]+|[,，、;；\s]+$/g, '')
  ...
  if (manual && /[×xX*]/.test(value)) return manual;

  const leading = value.match(
    new RegExp(`^(\\d+|[${CHINESE_NUMBER_CHARS}]+)\\s*(${QUANTITY_UNIT_PATTERN})\\s*(.+)$`, 'u')
  );
  if (leading) {
  ...
  const trailing = value.match(
    new RegExp(`^(.+?)\\s*(\\d+|[${CHINESE_NUMBER_CHARS}]+)\\s*(${QUANTITY_UNIT_PATTERN})$`, 'u')
  );
  if (trailing) {
...
function inferRoom(text: string, rooms: Pick<Room, 'id' | 'name'>[] = []) {
  const source = text.toLowerCase();
  return rooms
    .filter((room) => room.id !== GLOBAL_ROOM_ID && room.name.trim())
    .sort((a, b) => b.name.length - a.name.length)
    .find((room) => source.includes(room.name.toLowerCase()));
}

function stripNaturalPrefix(text: string, roomName?: string): string {
  let value = cleanText(text);
  if (roomName) {
    const roomPattern = escapeRegExp(roomName);
    value = value.replace(
      new RegExp(`^.*?${roomPattern}\\s*(?:里|裏|里面|裡面|内|中|这边|那里)?\\s*(?:有|放着|放了|新增|添加|加上|加了|录入|记录|买了)?\\s*`, 'u'),
      ''
    );
  } else {
    value = value.replace(
      /^(?:我|我们|家里|这边|这里|那个|嗯|呃|请帮我|帮我|帮我记一下)?\s*[^,，、;；\n]{0,30}?(?:里|裏|里面|裡面|内|中)\s*(?:有|放着|放了|新增|添加|加上|加了|录入|记录|买了)\s*/u,
      ''
    );
  }
  return value
...
function splitNaturalItems(text: string): string[] {
  const conjunctionPattern = new RegExp(
    `(还有|以及|另外|并且|再加上|加上|再有|\\s和\\s|\\s跟\\s|\\s与\\s|和(?=\\s*(?:\\d+|[${CHINESE_NUMBER_CHARS}]+)))`,
    'gu'
  );
...
function looksLikeNaturalQuickAdd(text: string, roomName?: string): boolean {
  ...
  if (/(还有|以及|另外|并且|再加上|再有)/u.test(value)) return true;
  return new RegExp(
    `(^|[,，、;；\\s])(?:\\d+|[${CHINESE_NUMBER_CHARS}]+)\\s*(?:${QUANTITY_UNIT_PATTERN})?\\S+`,
    'u'
  ).test(value);
}

function parseManualQuickAddLine(line: string): ParsedQuickAddLine | null {
  let name = line.trim();
  if (!name) return null;
  let note = '';
  let qty = 1;
...
export function parseQuickAddDraft(
  text: string,
  options: { rooms?: Pick<Room, 'id' | 'name'>[] } = {}
): ParsedQuickAddDraft {
  const source = text.trim();
  const room = inferRoom(source, options.rooms);
  if (!source) return { lines: [], roomId: room?.id, inferredRoomName: room?.name, mode: 'lines' };
...
export function formatQuickAddLines(lines: ParsedQuickAddLine[]): string {
  return lines
    .map((line) => {
