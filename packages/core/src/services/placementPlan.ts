...
function scoreCabinet(cabinet: Cabinet, item: Item, tags: string[], allItems: Item[]): number {
  ...
  for (const existing of allItems.filter((x) => x.cabinetId === cabinet.id && x.status !== 'pending')) {
    ...
    score += overlap * 4;
    if (existing.name && item.name && existing.name[0] === item.name[0]) score += 0.5;
  }
  return score;
}

export function suggestPlacementPlan(
  rooms: Room[],
  cabinets: Cabinet[],
  items: Item[],
  limit = 40
  ...
  const suggestions = pending.map((item): PlacementSuggestion => {
    ...
    if (item.roomId && item.roomId !== GLOBAL_ROOM_ID) {
      return {
        itemId: item.id,
...
export async function applyPlacementPlan(
  storage: Storage,
  plan: PlacementPlan,
  itemIds: string[]
): Promise<number> {
  ...
  for (const suggestion of plan.suggestions) {
    ...
    if (suggestion.targetCabinetId === '__global_loose__') {
      cabinet = await ensureGlobalLooseCabinet(storage);
      await storage.put('cabinets', cabinet);
    } else if (suggestion.targetCabinetId === '__room_loose__') {
      cabinet = await ensureLooseCabinet(storage, suggestion.targetRoomId);
      await storage.put('cabinets', cabinet);
    } else {
