...
import { ensureLooseCabinet } from './cabinet';
import type { DetectionResult, DetectedBox } from './ai';
import { logAction } from './actionLog';

type ScanCandidatePatch = Partial<
  Pick<
    ScanCandidate,
    | 'name'
    | 'reviewStatus'
...
function toCandidate(kind: ScanCandidate['kind'], box: DetectedBox): ScanCandidate {
...
function rectArea(rect: ScanCandidate['rect']): number {
  return Math.max(0, rect.w) * Math.max(0, rect.h);
}

function intersectionArea(a: ScanCandidate['rect'], b: ScanCandidate['rect']): number {
...
function rectCenter(rect: ScanCandidate['rect']) {
  return { x: rect.x + rect.w / 2, y: rect.y + rect.h / 2 };
}

function containsPoint(rect: ScanCandidate['rect'], point: { x: number; y: number }): boolean {
  return point.x >= rect.x && point.x <= rect.x + rect.w && point.y >= rect.y && point.y <= rect.y + rect.h;
}

function distanceFromPointToRect(rect: ScanCandidate['rect'], point: { x: number; y: number }): number {
...
function scorePlacement(cabinet: ScanCandidate, item: ScanCandidate) {
...
function placementReason(match: ReturnType<typeof scorePlacement>): string {
...
export function inferScanCandidatePlacements(candidates: ScanCandidate[]): ScanCandidate[] {
  const activeCabinets = candidates.filter(
    (candidate) => candidate.kind === 'cabinet' && candidate.reviewStatus !== 'rejected'
  );

  return candidates.map((candidate) => {
    if (candidate.kind !== 'item') return candidate;

    const manualTargetStillValid =
      candidate.placementSource === 'user' &&
      (!candidate.suggestedCabinetCandidateId ||
        activeCabinets.some((cabinet) => cabinet.id === candidate.suggestedCabinetCandidateId));
    if (manualTargetStillValid) return candidate;
...
export async function createScanSessionFromDetection(
  storage: Storage,
  photo: Photo,
...
export async function updateScanCandidate(
  ...
  sessionId: string,
  candidateId: string,
  patch: ScanCandidatePatch
): Promise<ScanSession> {
  const session = await storage.get('scanSessions', sessionId);
  if (!session) throw new Error('扫描会话不存在');
  const candidates = session.candidates.map((candidate) =>
    candidate.id === candidateId
      ? {
          ...
          ...patch,
          reviewStatus:
            patch.reviewStatus || (patch.name || patch.rect ? 'edited' : candidate.reviewStatus),
        }
      : candidate
  );
...
export async function applyScanSession(
  ...
  sessionId: string,
  acceptedIds?: string[]
): Promise<{ cabinets: Cabinet[]; items: Item[] }> {
  const session = await storage.get('scanSessions', sessionId);
  if (!session) throw new Error('扫描会话不存在');
  if (session.status === 'applied') throw new Error('这个扫描会话已经应用过');
  const photo = await storage.get('photos', session.photoId);
  if (!photo) throw new Error('来源照片不存在');
  ...
  const items: Item[] = [];
  const cabinetIdByCandidateId = new Map<string, string>();
  ...
  const itemCandidates = reviewedCandidates.filter(
    (candidate) => candidate.kind === 'item' && accepted.has(candidate.id)
  );
  if (itemCandidates.length) {
    let loose: Cabinet | undefined;
    for (const candidate of itemCandidates) {
      const targetCabinet = candidate.suggestedCabinetCandidateId
        ...
        : undefined;
      if (!targetCabinet) {
        loose = loose || (await ensureLooseCabinet(storage, photo.roomId));
        await storage.put('cabinets', loose);
      }
      const cabinet = targetCabinet || loose!;
      const crop = await cropItemFromPhoto(photo.blob, candidate.rect);
      ...
      const item: Item = {
        ...
        note: '',
        tags: [],
        image: crop || (await generateItemThumb(candidate.name, candidate.emoji || 'box')),
  ...
  const next: ScanSession = { ...session, candidates: reviewedCandidates, status: 'applied', appliedAt: Date.now() };
  await storage.put('scanSessions', next);
...
export async function discardScanSession(storage: Storage, sessionId: string): Promise<void> {
  const session = await storage.get('scanSessions', sessionId);
  if (!session) return;
