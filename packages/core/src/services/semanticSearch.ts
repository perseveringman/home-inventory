...
function itemText(item: Item, room?: Room, cabinet?: Cabinet): string {
  return [
    item.name,
    item.note,
    item.tags?.join(' '),
...
export function semanticSearchInventory(
  ...
  for (const item of items) {
    ...
    if (item.name.toLowerCase().includes(query.trim().toLowerCase())) score += 8;
    if (item.tags?.some((tag) => query.includes(tag))) score += 4;
    if (item.status === 'placed') score += 0.5;
    if (score > 0) {
      results.push({
        item,
        room,
